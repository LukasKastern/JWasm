...
static void freeEntry(entry_ptr tr, _trmem_hdl hdl) { hdl->free(tr); }
...
_trmem_hdl _trmem_open(void *(*alloc)(size_t), void (*free)(void *),
                       void *(*realloc)(void *, size_t),
                       void *(*expand)(void *, size_t), FILE *prt_parm,
                       void (*prt_line)(FILE *, const char *, size_t),
                       unsigned flags)
/*****************************************************/
{
...
static int isValidChunk(entry_ptr tr, const char *rtn, _trmem_who who,
                        _trmem_hdl hdl) {
  void *mem;
  size_t size;
  size_t blk_size;
...
static void *ChangeAlloc(void *old, size_t size, _trmem_who who, _trmem_hdl hdl,
                         void *(*fn)(void *, size_t), char *name)
/*********************************************************************/
{
  entry_ptr tr;
  void *new_block;
...
/**********************************************/
{
  ...
  for (;;) {
    if (tr == 0) {
      trPrt(hdl, MSG_NOT_IN_ALLOCATION, "ChkRange", who, start);
      return (0);
    }
    end_of_mem = _PtrAdd(tr->mem, getSize(tr));
    if (_PtrCmp(start, >=, tr->mem) && _PtrCmp(start, <, end_of_mem))
      break;
    tr = tr->next;
  }
  end = _PtrAdd(start, len);
  if (_PtrCmp(end, >, end_of_mem)) {
    trPrt(hdl, MSG_OVERRUN_2, "ChkRange", who, start, len, tr->mem,
          getSize(tr));
    return (0);
  }
...
/****************************************/
{
  ...
  do {
    size = getSize(tr);
    if (chunks < 20) {
      trPrt(hdl, MSG_PRT_LIST_3, tr->who, tr->mem, size, tr->when, tr->mem,
            size);
    }
    ++chunks;
    tr = tr->next;
...
/******************/
{
  if ((FileTrmem = fopen(TRMEM_LOGFN, "w"))) {
    ...
    hTrmem = _trmem_open(malloc, free, _TRMEM_NO_REALLOC, _TRMEM_NO_REALLOC,
                         FileTrmem, memLine,
